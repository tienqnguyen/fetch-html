export default {
  async fetch(request: Request): Promise<Response> {
    try {
      ...
      const regexParam = incoming.searchParams.get("regex");
      ...
      // Get response body
      let responseBody = remoteResponse.body;
      
      // Strip HTML tags if regex parameter is provided
      if (regexParam) {
        try {
          const text = await remoteResponse.text();
          const stripRegex = new RegExp(regexParam);
          const strippedText = text.replace(/<[^>]*>/g, '');
          responseBody = strippedText;
          headers.set("Content-Type", "text/plain; charset=utf-8");
        } catch (regexErr) {
          console.error("Regex error:", regexErr);
          return new Response("Invalid regex parameter", { status: 400 });
        }
      }
